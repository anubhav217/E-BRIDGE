...
//query
$result = mysql_query("SELECT * FROM `timetable` ") or die ('Query is invalid: ' . mysql_error());

// check for empty result
if (mysql_num_rows($result) > 0) {
    // looping through all results
    // products node
    $response["timetabledetails"] = array();
 //$row = mysql_fetch_array($result);
   while ($row = mysql_fetch_array($result)) {
        ...
        array_push($response["timetabledetails"], $product);
    ...
    // no products found
    $response["success"] = 0;
    $response["timetabledetails"] = "Invalid details";
